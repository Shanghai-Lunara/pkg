...
func (nf *nodeFilter) addDirtyWords(text string) {
	n := nf.root
	uchars := []rune(strings.ToLower(text))
...
func (nf *nodeFilter) FilterReaderResult(reader io.Reader, excludes ...rune) (map[string]int, error) {
	...
	for {
		...
		if ur >=32 && ur <=126 && !unicode.IsLetter(ur) && !unicode.IsNumber(ur){
			continue
		}
...
func (nf *nodeFilter) doFilter(uchars []rune, data map[string]int) {
	...
	for i := 0; i < ul; i++ {
		uri := uchars[i]
		if unicode.IsLetter(uri) {
			uri = unicode.ToLower(uri)
		}
		if _, ok := n.child[uri]; !ok {
			continue
		}
		n = n.child[uri]
		buf.WriteRune(uri)
		...
		for j := i + 1; j < ul; j++ {
			urj := uchars[j]
			if unicode.IsLetter(urj) {
				urj = unicode.ToLower(urj)
			}
			if _, ok := n.child[urj]; !ok {
				continue
			}
			n = n.child[urj]
			buf.WriteRune(urj)
...
func (nf *nodeFilter) doIndexes(uchars []rune, excludes ...rune) (idexs []int) {
	...
	for i := 0; i < ul; i++ {
		uri := uchars[i]
		if unicode.IsUpper(uri) {
			uri = unicode.ToLower(uri)
		}
		if nf.checkExclude(uri, excludes...) {
			continue
		}
		...
		for j := i + 1; j < ul; j++ {
			urj := uchars[j]
			if unicode.IsUpper(urj) {
				uri = unicode.ToLower(urj)
			}
			if nf.checkExclude(urj, excludes...) {
